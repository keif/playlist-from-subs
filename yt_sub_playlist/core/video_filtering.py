...
class VideoFilter:
    ...
    def filter_videos(
        ...
        for video in videos:
            if not self._should_include_video(video, channel_whitelist, min_duration):
                continue
        ...
        self._log_filtering_stats(channel_whitelist, min_duration)
        return filtered
    
    def _should_include_video(
        self, 
        video: Dict[str, Any], 
        channel_whitelist: Optional[Set[str]], 
        min_duration: int
    ) -> bool:
        """
        ...
        Args:
            ...
            channel_whitelist: Set of allowed channel IDs
    ...
    def _log_filtering_stats(self, channel_whitelist: Optional[Set[str]], min_duration: int) -> None:
        ...
        if channel_whitelist:
            logger.info(f"  Not in whitelist: {self.stats['not_whitelisted']}")
